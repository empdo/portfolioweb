...
export const getStaticProps: GetStaticProps = async (context) => {
  ...
  const mdxSource = await serialize(content, {mdxOptions: {remarkPlugins: [remarkGfm], rehypePlugins: [rehypeHighlight]}});
...
const PostPage = ({ frontmatter, mdxSource, posts }) => {
  ...
  return (
    <div className='flex flex-col items-start sm:mx-auto mx-4 max-w-screen-sm'>
      ...
      <Link href={{pathname: "/", query: { key: 'blog' } }} className="flex align-center">
        <BackArrow url="/" className="h-8 mr-6 fill-white inline" />
        ...
        <h1 className='text-xl mb-3 font-bold '>Table of content</h1>
          {
            toc.map((content, index) =>
              ...
              (index == 0 ? 'text-purple-300 marker:text-purple-300' : '')}
              key={index}>
      ...
      <div className='prose prose-invert pb-12' ref={contentRef}>
